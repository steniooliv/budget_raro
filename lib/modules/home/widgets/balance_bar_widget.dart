...
class BalanceBarWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (entryValue + outValue > 0) {
      entryBarWidth =
          entryValue > outValue ? maxWidht : (entryValue / outValue) * maxWidht;
      outBarWidth =
          entryValue < outValue ? maxWidht : (outValue / entryValue) * maxWidht;
    } else {
      entryBarWidth = 0;
      outBarWidth = 0;
    }
    ...
    return Column(
      children: [
        GestureDetector(
          child: barWidget("Saídas", outValueString, outBarWidth,
              AppColors.cyan, textSizeCalculator),
          onTap: () {}, // NAVEGAR PARA A TELA TRANSAÇÕES SAÍDAS
        ),
        GestureDetector(
          child: barWidget("Entradas", entryValueString, entryBarWidth,
...
Widget barWidget(String label, String value, double barWidth, Color color,
